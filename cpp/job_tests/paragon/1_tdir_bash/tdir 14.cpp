...
int main() {
	try {
		...
		while (1) {																	// In an endless loop, keep entering 'tdir [path/mask] [-r]' to list files and directories or 'Ctrl+C' to quit.
			...
			case (int)arguments::tdir :										// One parameter entered. That must be the tdir command. List without recursion. //TO DO: Introduce an enumeration for cases.
				listRecursively(QDir::current(), maskAll, false);
				break;
			case (int)arguments::tdirPathMask:								// Two parameters entered. The second parameter can be either a path or a recursion key.
				if (strcmp(pathRKey1, recurKey) == 0)						// Check if the second parameter is a recursion key or not.
					listRecursively(QDir::current(), maskAll, true);	// The second parameter is a recursion key. List recursively.
				else {
				...
				break;
			case (int)arguments::tdirPathMaskRkey: {						// All the three parameters entered.
