...
int main() {
	try {
		enum struct argumentCount : int {tdir = 1, tdirPathMask, tdirPathMaskRkey}; // Acceptable number of arguments: 1) tdir, 2) tdir path/mask, 3) tdir path/mask -r.
		const char* const cmdTdir = "tdir";	// The tdir command.
		const char* const recurKey = "-r";	// The recursion key.
		const QString cmdExit("exit");		// The exit command. //TO DO: Introduce this command.
		...
		while (1) {																	// In an endless loop, keep entering 'tdir [path/mask] [-r]' to list files and directories or 'Ctrl+C' to quit.
			...
			if (strlen(line) < strlen(cmdTdir)) {								// The first argument must be the tdir command. First check it just by its length.
			...
			if (strcmp(command, cmdTdir) != 0) {								// Check the tdir command first.
				fprintf(stderr, badCommand);									// Bad command.
				continue;
			}

			switch (argumentsRead) {											// Parse the command line based on the number of arguments successfully read.
			case (int)argumentCount::tdir :									// One argument entered. That must be the tdir command. List without recursion. //TO DO: Introduce an enumeration for cases.
				listRecursively(QDir::current(), maskAll, false);
				break;
			case (int)argumentCount::tdirPathMask:							// Two arguments entered. The second argument can be either a path or a recursion key.
				...
				break;
			case (int)argumentCount::tdirPathMaskRkey: {					// All the three arguments entered.
