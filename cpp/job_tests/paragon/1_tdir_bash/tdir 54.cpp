	...
	try {
		...
		while (1) {																				// In an endless loop, keep entering 'tdir [path/mask] [-r]' to list files and directories or 'Ctrl+C' to quit.
			...
			if (wordsRead == EOF || wordsRead == 0) {									// An input failure.
				...
				continue;
			} else if (wordsRead > (int)wordCount::tdir_path_rkey) {				// Redundant words entered. //TO DO: Check the first word. If that is tdir then comment on its wrong arguments otherwise on a wrong command.
