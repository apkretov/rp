...
int main() {
	try {
		...
		while (1) {														// In an endless loop, keep entering 'tdir [path/mask] [-r]' to list files and directories or 'Ctrl+C' to quit.
			...
			switch (itemsRead) {
//			case 1:														// Only one item entered. That must be the tdir command. List without recursion.
//				if (QString(chrCommand) == cmdTdir) {			// Check the tdir command first.
//					listRecursively(currentDir, mask, false);
//				} else													// Bad command.
//					fprintf(stderr, errComment);
//				break;
...
//					fprintf(stderr, errComment);
//				break;
			case 1: case 2:											// //////////////Only one item entered. That must be the tdir command. List without recursion.
				if (QString(chrCommand) != cmdTdir) {			// Check the tdir command first.
