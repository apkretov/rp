...
int main() {
	try {
		...
		char chrCommand[commandLength];											// Either of the three entered items might be of the maximal lenght.
...
//		char chrMask[cmdLength];
		...
		while (1) {														// In an endless loop, keep entering 'tdir [path/mask] [-r]' to list files and directories or 'Ctrl+C' to quit.
			...
			int itemsRead = sscanf(line, "%s %s %s", chrCommand, pathMask1, pathMask2); //TO DO: Use sscan_s instead.
...
//					listRecursively(QDir(path), mask, false);
			case 2:														// Two items entered. The second item can be either a path or a recursion key.
					///////////if (strcmp(pathMask1, recurKey) == 0)		// The second
					QString path(pathMask1);						// Create a path entered.
					listRecursively(QDir(path), mask, false);
				break;
			}
		}
