...
int main() {
	try {
		...
		constexpr unsigned commandLength = 1024;							// The command maximum lengh.
		char line[commandLength];
		char command[commandLength];											// Either of the three entered parameters might be of the maximal lenght.
//		char chrPath[cmdLength];												// That's why reserve the maximal space for all the three.
//		char chrMask[cmdLength];
		char pathMask1[commandLength];										// That's why reserve the maximal space for all the three.
		char pathMask2[commandLength];
		...
		while (1) {															// In an endless loop, keep entering 'tdir [path/mask] [-r]' to list files and directories or 'Ctrl+C' to quit.
			...
			if (fgets(line, sizeof(line), stdin) == NULL)
				exit(0);
			if (strlen(line) < strlen(chrTdir)) {					// The first parameter must be the tdir command. First check it just by its length.
			...
			case 1:															// Only one parameter entered. That must be the tdir command. List without recursion. //TO DO: Introduce an enumeration for cases.
				...
				break;
//			case 2:															// Two parameters entered. Those must be the tdir command and a path. List without recursion.
