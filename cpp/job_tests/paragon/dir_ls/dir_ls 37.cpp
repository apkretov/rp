...
int main() {
	try {
		...
		while (1) {															// In an endless loop, keep entering 'tdir [path/mask] [-r]' to list files and directories or 'Ctrl+C' to quit.
			...
			int paramsRead = sscanf(line, "%s %s %s", command, pathMask1, pathMask2);  //Read parameters. Get the number of them successfully read. //TO DO: Use sscan_s instead.
			...
			switch (paramsRead) {										// Parse the command line based on the number of parameters successfully read. //TO DO: Check if a directory entered exists.
...
//				}
				if (strcmp(pathMask1, recurKey) != 0) {			// Check if the second parameter is a recursion key or not.
					QString path(pathMask1);							// The second parameter should be a path.
					listRecursively(QDir(path), mask, false);		// List without recursion.
				}
				else
