	...
	try {
		...
		while (1) {															// In an endless loop, keep entering 'tdir [path/mask] [-r]' to list files and directories or 'Ctrl+C' to quit.
			...
			case 3: {														// All the three parameters entered.
				...
				int comparison1 = strcmp(pathRKey1, recurKey);	// Check if the second parameter is a recursion key or not.
				if (comparison1 != 0)
				...
				if (comparison1 == 0) 									// The third parameter a recursion key. List recursively.
					recursive = true;										// The second parameter is a recursion key. List recursively.
