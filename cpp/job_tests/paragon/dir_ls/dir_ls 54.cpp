...
bool retrieveMask(const QString& path, QString& mask) {
	...
	int slashPos = path.lastIndexOf(rightSlash);					// Find the position of the last slash in the path.
	//ERROR: A path might be completely a mask.
	if (slashPos == -1)													// No slash found.
		//return mask;														// An empty mask.
		return false;
	else {																	// Continue, if found.
		QString pathSlashed = path.mid(slashPos + 1);			// The section of the path after the last slash.
		////////////
		//return mask;
		return true;
	}
