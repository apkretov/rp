...
void distinguishPathAndMask(QString&, QString&);

//const QString currentDir2(".");										// The current directory.
...
int main() {
	try {
		...
		while (1) {															// In an endless loop, keep entering 'tdir [path/mask] [-r]' to list files and directories or 'Ctrl+C' to quit.
			...
			path = "";														// Reset path and mask before getting newly entered values in each cycle.
			mask = "";
			...
			case 1:															// Only one parameter entered. That must be the tdir command. List without recursion. //TO DO: Introduce an enumeration for cases.
				listRecursively(currentDir, maskAll, false);
				break;
			case 2:															// Two parameters entered. The second parameter can be either a path or a recursion key.
				if (strcmp(pathRKey1, recurKey) == 0)				// Check if the second parameter is a recursion key or not.
					listRecursively(currentDir, maskAll, true);	// The second parameter is a recursion key. List recursively.
				else {
					path = QString(pathRKey1);							// The second parameter is a path.
					//distinguishPathAndMask(path, maskAll);			// If included, retrieve a mask from the end of the path.
					QString mask = maskAll;
			...
			case 3: {														// All the three parameters entered.
				...
				listRecursively(QDir(path), maskAll, true);			// List recursively. //TO DO: Check if a directory entered exists. //TO DO: Parse to cut out a mask, if any.
				break;
			}
			}
		}
...
//*********************************************************************************************************************************************************
// TO DO: Update the description based on the new function name.
...
//*********************************************************************************************************************************************************
void distinguishPathAndMask(QString& path, QString& mask) {
