...
//			}
			...
			case (int)argumentCount::tdir_exit :									// One argument entered. That must be the tdir command. List without recursion. //TO DO: Introduce an enumeration for cases.
//				if (strcmp(command, cmdTdir) != 0 && strcmp(command, cmdExit) != 0) { // Check a tdir/exit command first.
//					fprintf(stderr, badCommand);								// Bad command.
//					continue;
//				}
