...
int main() {
	try {
		enum struct wordCount : int {tdir_exit = 1, tdir_path, tdir_path_rkey};	// Acceptable number of command words: 1) tdir/exit, 2) tdir path/mask, 3) tdir path/mask -r.
		...
		out  << "The " << cmdTdir << " File Listing Utility Program" << endl					// The introduction.
			 << "List files:\t'tdir [path/mask] [-r]' or 'tdir [-r] [path/mask]'" << endl	//TO DO: Print the maximal length restriction.
		...
		while (1) {																				// In an endless loop, keep entering 'tdir [path/mask] [-r]' to list files and directories or 'Ctrl+C' to quit.
			...
			int argumentsRead = sscanf(line, "%s %s %s %s", command, pathRKey1, pathRKey2, redundant);  // Get the number of successfully read words. //TO DO: Use sscan_s instead.
			if (argumentsRead == EOF || argumentsRead == 0) {						// Input failure.
				fprintf(stderr, "No valid input!\n\n");
				continue;
			} else if (argumentsRead > (int)wordCount::tdir_path_rkey) {		// Redundant words entered. //TO DO: Check the first argument. If that is tdir then comment on its wrong arguments otherwise on a wrong command.
			...
			switch (argumentsRead) {														// Parse the command line based on the number of words successfully read.
			case (int)wordCount::tdir_exit :												// One argument entered. That must be a tdir command or an exit one.
				listRecursively(QDir::current(), maskAll, false);					// List without recursion.
				break;

			case (int)wordCount::tdir_path:												// Two words entered. The second argument can be either a path or a recursion key.
			...
			case (int)wordCount::tdir_path_rkey: {										// Three words entered.
				if (strcmp(pathRKey1, pathRKey2) == 0) {								// The second and the third words must not be the same.
					fprintf(stderr, "The second and the third words must not be the same!\n\n"); // Bad command.
					continue;
				}
