...
int pipe_create(pipe_t* sttPipe, int intStages) { //Part 4 shows pipe_create, the function that creates a pipeline. It can create a pipeline of any number of stages, linking them together in a list.			//External interface to create a pipeline. All the data is initialized and the threads created. They'll wait for data.
	...
	for (intPipeIndex = 0; intPipeIndex <= intStages; intPipeIndex++) { 	//18-34 For each stage, it allocates a new stage_t structure and initializes the members. Notice that one additional "stage" is allocated and initialized to hold the final result of the pipeline.
