...
void* alarm_thread(void* arg) { //The alarm thread's start routine.
	...
	while (1) { //Loop forever, processing commands. The alarm thread will be disintegrated when the process exits.
		...
		//Unlock the mutex before waiting, so that the main thread can lock it to insert a new alarm request. If the sleep_time is 0, then call sched_yield, giving the main thread a chance to run if it has been readied by user input, without delaying the message if there's no input.
		status = pthread_mutex_unlock (&alarm_mutex); //52-58: The mutex is always unlocked before sleeping or yielding. If the mutex remained locked, then main would be unable to insert a new alarm on the list. That would make the program behave synchronously—the user would have to wait until the alarm expired before doing anything else.
		if (status != 0) {
...
int main(int argc, char *argv[]) { //Each request is queued to a server thread, alarm_thread. As soon as main has queued the request, it is free to read the next command.
	...
	status = pthread_create(&thread, NULL, alarm_thread, NULL); //8-11: Create the server thread that will process all alarm requests.
	if (status != 0)
		err_abort (status, "Create alarm thread"); //11
	while (1) {
		...
		if (sscanf(line, "%d %64[^\n]", &alarm->seconds, alarm->message) < 2) { //Parse input line into seconds (%d) and a message (%64[^\n]), consisting of up to 64 characters separated from the seconds by whitespace.
			...
			status = pthread_mutex_lock(&alarm_mutex); //30-32: The program needs to add the alarm request to alarm_list, which is shared by both alarm_thread and main. So we start by locking the mutex that synchronizes access to the shared data, alarm_mutex.
			if (status != 0)
				err_abort(status, "Lock mutex"); //32
			...
			last = &alarm_list; //39-49: The alarms are sorted in order of expiration time on the alarm_list queue.
			next = *last;
