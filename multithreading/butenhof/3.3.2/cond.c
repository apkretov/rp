...
int main(int argc, char* argv[]) { //Protects access to value. Signals change to value. Access protected by mutex.
	...
	status = pthread_mutex_lock(&data.mutex); if (status != 0) err_abort(status, "Lock mutex"); printf("main 2 \t\t After pthread_mutex_lock. \n"); //TEST

	while (data.value == 0) { printf("main 3 \t\t Before pthread_cond_wait. \n"); //TEST
		status = pthread_cond_timedwait(&data.cond, &data.mutex, &timeout); /*TEST status = pthread_cond_wait(&data.cond, &data.mutex);*/ printf("main 4 \t\t After pthread_cond_wait. \n"); //TEST
		if (status == ETIMEDOUT) {
	...
	status = pthread_mutex_unlock(&data.mutex); if (status != 0) err_abort(status, "Unlock mutex"); printf("main 6 \t\t After pthread_mutex_unlock. \n"); //TEST
